...
#ifdef __APPLE__
string timing::vFont = "/System/Library/Fonts/Helvetica.dfont";
#else
...
static int vCellWtr=16; // Proportional width of a transition
...
static void draw_transition (gc &gc, int x, int y, const sigvalue &last,
  ...
  case ZERO:
    ...
    case Z:
      gc.line (x, y + vCellHt/2, x + vCellW/vCellWtr/2, y + vCellH);
  ...
  case Z:
    ...
    case TICK:
    case PULSE:
      gc.line (x, y + vCellH, x + vCellW/vCellWtr/2, y + vCellHt/2);
      gc.line (x + vCellW/vCellWtr/2, y + vCellHt/2, x + vCellW, y + vCellHt/2);
      break;

    case ONE:
      gc.line (x, y + vCellHsep, x + vCellW/vCellWtr/2, y + vCellHt/2);
      gc.line (x + vCellW/vCellWtr/2, y + vCellHt/2, x + vCellW, y + vCellHt/2);
      break;
...
static void draw_dependency (gc &gc, int x0, int y0, int x1, int y1) {
  Magick::CoordinateList shaft, head;
...
void postscript_gc::bezier (const Magick::CoordinateList &points) {
  Magick::CoordinateList::const_iterator i = points.begin();
